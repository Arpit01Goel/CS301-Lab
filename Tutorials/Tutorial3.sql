...
-- select studentid, count(*) from takes group by studentid having count(course_id) >=2;
...
-- select distinct name from student where studentid in (select distinct studentid from student natural join takes join course on course.courseId = takes.course_id where student.dept_name = course.dept_name);

-- select name from student where studentid in ( select studentid from takes group by takes.studentid, takes.grade having count(grade) =1 and grade='A');
...
-- select studentid, count(course_id) from takes group by studentid order by studentid asc;
